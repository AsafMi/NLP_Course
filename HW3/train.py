...
def train(training_args):
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=training_args.learning_rate, steps_per_epoch=len(train_dataloader),
    ...
    for epoch in range(training_args.num_epochs):
        ...
        if training_args.do_train:
            train_loop(train_dataloader, model, loss_fn, optimizer, scheduler, device, epoch, training_args)
...
def train_loop(dataloader, model, loss_fn, optimizer, scheduler, device, epoch, training_args):
    model.train()
